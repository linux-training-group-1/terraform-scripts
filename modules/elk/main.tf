...
resource "google_compute_instance" "elk_instance" {
  ...
  // setup the SSH metadata for ansible
  metadata = {
...
#    command = "ansible-playbook -i '${google_compute_instance.elk_instance.network_interface.0.access_config.nat_ip},' --private-key ${var.ssh_params.private_path} ../ansible/${var.ansible_playbook}"
#  }
}